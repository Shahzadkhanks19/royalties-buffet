  ...
  return (
    <>
      <section className={internalHero}>
        <img src="https://images.unsplash.com/photo-1515003197210-e0cd71810b5f?auto=format&fit=crop&w=2200&q=90" alt="Premium restaurant table setting" className="absolute inset-0 h-full w-full object-cover" />
      ...
      <section className="bg-[#0a0a0a] py-20 text-white lg:py-24">
        <div className={`${shell} grid gap-8 xl:grid-cols-[1.15fr_.85fr]`}>
          <form onSubmit={handleSubmit} className="border border-white/10 bg-[#0d0d0d] p-5 sm:p-7 lg:p-9">
            ...
            <div className="mt-7"><span className="mb-3 block text-[0.58rem] font-black uppercase tracking-[0.14em] text-white/45">Select date</span><div className="grid grid-cols-4 gap-2 sm:grid-cols-7">{dates.map((date) => { const active = form.date === date.value; return <button key={date.value} type="button" onClick={() => update("date", date.value)} className={`min-h-24 border px-2 py-3 text-center transition ${active ? "border-[#d8ab4d] bg-[#d8ab4d] text-black" : "border-white/10 bg-white/[0.02] text-white hover:border-[#d8ab4d]/45 hover:bg-[#d8ab4d]/5"}`}><span className="block text-[0.56rem] font-black uppercase tracking-[0.12em] opacity-60">{date.day}</span><strong className="mt-1 block font-serif text-2xl">{date.number}</strong><span className="text-[0.58rem] uppercase tracking-[0.1em] opacity-60">{date.month}</span></button>; })}</div></div>

            <div className="mt-7"><span className="mb-3 block text-[0.58rem] font-black uppercase tracking-[0.14em] text-white/45">Select time</span><div className="grid grid-cols-2 gap-2 sm:grid-cols-5">{times.map((time) => { const active = form.time === time; return <button key={time} type="button" onClick={() => update("time", time)} className={`min-h-12 border px-3 text-sm font-bold transition ${active ? "border-[#d8ab4d] bg-[#d8ab4d]/12 text-[#efc86f]" : "border-white/10 bg-white/[0.02] text-white/66 hover:border-[#d8ab4d]/40 hover:text-white"}`}>{time}</button>; })}</div></div>
            ...
            <div className="mt-7 grid gap-5 md:grid-cols-2">
              ...
              <label className="block md:col-span-2"><span className="mb-2 block text-[0.58rem] font-black uppercase tracking-[0.14em] text-white/45">Special requests</span><textarea value={form.requests} onChange={(event) => update("requests", event.target.value)} placeholder="High chair, celebration note, accessibility needs, seating preference..." rows={4} className="w-full resize-none border border-[#d8ab4d]/28 bg-black/55 px-4 py-3 text-sm leading-7 text-white outline-none transition placeholder:text-white/28 focus:border-[#d8ab4d]/65" /></label>
            </div>
          ...
          <aside className="space-y-5">
            <div className="sticky top-32 border border-[#d8ab4d]/25 bg-[#15120c] p-6 sm:p-7">
      ...
      {submitted ? <section className="bg-[#d8ab4d] py-16 text-[#120f09]"><div className={`${shell} flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between`}><div className="flex items-start gap-4"><div className="grid size-12 shrink-0 place-items-center bg-black text-[#d8ab4d]"><Check className="size-5" /></div><div><p className="text-[0.62rem] font-black uppercase tracking-[0.16em] opacity-60">Request received</p><h2 className="mt-1 font-serif text-3xl">Your reservation request is ready for confirmation.</h2></div></div></div></section> : null}
    </>
  );
}
