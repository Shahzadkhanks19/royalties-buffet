...
function Hero() {
  return (
    <section className="relative overflow-hidden bg-[#070707] pt-[88px] text-white lg:pt-[108px]">
...
function ReservationStrip() {
  ...
  return <section className="relative overflow-visible bg-[#0a0a0a] py-16 text-white lg:py-20"><img src="https://images.unsplash.com/photo-1414235077428-338989a2e8c0?auto=format&fit=crop&w=2200&q=80" alt="Elegant dining table" loading="lazy" className="absolute inset-0 h-full w-full object-cover opacity-25" /><div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(5,5,5,.96),rgba(5,5,5,.85),rgba(5,5,5,.92))]" /><div className={`${shell} relative z-10 grid gap-8 lg:grid-cols-[.8fr_1.2fr] lg:items-end`}><div><p className="text-[0.64rem] font-black uppercase tracking-[0.24em] text-[#d8ab4d]">Reserve your experience</p><h2 className="mt-3 font-serif text-[clamp(3rem,5vw,5rem)] leading-[0.95]">Book your table now.</h2><p className="mt-4 max-w-lg text-sm leading-7 text-white/48">Choose your date, time and party size. The full reservation workflow will connect to the Express/MongoDB backend.</p></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><CustomSelect label="Date" value={date} placeholder="Select Date" options={["Today","Tomorrow","This Weekend"]} icon={CalendarDays} onChange={setDate} /><CustomSelect label="Time" value={time} placeholder="Select Time" options={["12:30 PM","2:00 PM","7:00 PM","8:30 PM"]} icon={Clock3} onChange={setTime} /><CustomSelect label="Guests" value={guests} placeholder="2 Guests" options={["1 Guest","2 Guests","3 Guests","4 Guests","5+ Guests"]} icon={Users} onChange={setGuests} /><Link to="/reservation" className={`${buttonGold} self-end`}>Book Now</Link></div></div></section>;
}

export default function HomePage() {
  return <><Hero /><Signature /><AboutOccasions /><PromoPanels /><Gallery /><ReservationStrip /></>;
}
