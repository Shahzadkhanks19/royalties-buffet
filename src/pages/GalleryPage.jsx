  ...
  return (
    <>
      <section className={internalHero}>
        <img src="https://images.unsplash.com/photo-1414235077428-338989a2e8c0?auto=format&fit=crop&w=2200&q=90" alt="Premium restaurant dining experience" className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className={shell}>
          ...
          <div className="mt-10 flex flex-wrap gap-2" role="tablist" aria-label="Gallery categories">
            {filters.map((filter) => {
              ...
              return (
                <button key={filter} type="button" role="tab" aria-selected={active} onClick={() => setActiveFilter(filter)} className={`min-h-11 border px-4 text-[0.62rem] font-black uppercase tracking-[0.13em] transition duration-300 ${active ? "border-[#17130e] bg-[#17130e] text-[#f1cf77] shadow-lg" : "border-[#8d7447]/25 bg-[#fffaf2] text-[#735b30] hover:-translate-y-0.5 hover:border-[#a97c29] hover:text-[#17130e]"}`}>
                  {filter}
                </button>
              );
            })}
      ...
      {activeItem ? (
        <div className="fixed inset-0 z-[100] grid place-items-center bg-black/94 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={activeItem.title}>
          <button type="button" onClick={() => setActiveItem(null)} className="absolute right-4 top-4 grid size-12 place-items-center border border-white/20 bg-black/70 text-white transition hover:border-[#d8ab4d] hover:bg-[#d8ab4d] hover:text-black sm:right-6 sm:top-6" aria-label="Close gallery image"><X className="size-5" /></button>
          <div className="w-full max-w-6xl overflow-hidden border border-white/10 bg-[#0b0b0b] shadow-2xl">
            <div className="relative max-h-[72vh] min-h-[420px]"><img src={activeItem.image} alt={activeItem.title} className="h-full max-h-[72vh] w-full object-contain" /></div>
