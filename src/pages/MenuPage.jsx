...
import { menuImageFallback, menuItems as seededMenuItems } from "../data/menuItems";
import useCmsCollection from "../hooks/useCmsCollection";

const preferredCategoryOrder = ["Live Grill", "Starters", "Indian", "Regional Indian", "Italian", "Indo-Chinese", "Mexican", "Japanese", "Middle Eastern", "Global", "Desserts", "Beverages"];
...
function FoodType({ type, protein }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 text-[0.58rem] font-black uppercase tracking-[0.12em] ${veg ? "text-emerald-700" : "text-red-700"}`}>
      ...
      {veg ? "Veg" : `Non-Veg · ${protein || "Selection"}`}
    </span>
  );
}
...
function MenuImage({ item }) {
  ...
  return <img src={item.image} alt={item.title} loading="lazy" onError={handleError} className="h-full w-full object-cover transition duration-700 group-hover:scale-110" />;
}

export default function MenuPage() {
  const [activeCategory, setActiveCategory] = useState("All");
  const { items: menuItems } = useCmsCollection("/api/menu", seededMenuItems);

  const categories = useMemo(() => {
    const available = [...new Set(menuItems.map((item) => item.category).filter(Boolean))];
    const ordered = preferredCategoryOrder.filter((category) => available.includes(category));
    const extras = available.filter((category) => !preferredCategoryOrder.includes(category)).sort();
  ...
  return (
    <>
      <section className={internalHero}>
        ...
        <div className={internalHeroInner}><div className="max-w-3xl"><p className="text-[0.7rem] font-black uppercase tracking-[0.3em] text-[#d8ab4d]">The Royalties Buffet</p><h1 className="mt-5 font-serif text-[clamp(4rem,8vw,7.5rem)] leading-[0.86] tracking-[-0.05em]">A world of flavour.<br /><span className="text-[#d8ab4d]">One grand spread.</span></h1><p className="mt-7 max-w-2xl text-base leading-8 text-white/68 sm:text-lg">Explore regional Indian favourites alongside Italian, Indo-Chinese, Mexican, Japanese, Middle Eastern and other popular global flavours, plus live grills and an indulgent dessert finish.</p></div></div>
      </section>
      ...
      <section className="bg-[#f3ecdf] py-24 lg:py-28">
        <div className={shell}>
          <div className="flex flex-col justify-between gap-7 lg:flex-row lg:items-end"><div className="max-w-3xl"><p className="text-[0.68rem] font-black uppercase tracking-[0.28em] text-[#9d7328]">Explore the spread</p><h2 className="mt-4 font-serif text-[clamp(3rem,5vw,5.2rem)] leading-[0.94] tracking-[-0.04em] text-[#17130e]">Familiar favourites. More to discover.</h2></div><p className="max-w-md text-sm leading-7 text-[#6c6254]">A broad India-focused buffet mix with popular international cuisines. Non-vegetarian selections remain limited to chicken, mutton, eggs and seafood.</p></div>
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-4">{filteredItems.map((item) => <article key={item._id || `${item.category}-${item.title}`} className="group overflow-hidden border border-black/10 bg-[#fffaf2] shadow-[0_14px_35px_rgba(36,24,10,.08)] transition duration-500 hover:-translate-y-2 hover:border-[#c9983d]/55 hover:shadow-[0_24px_55px_rgba(36,24,10,.15)]"><div className="relative h-56 overflow-hidden"><MenuImage item={item} /><div className="absolute inset-0 bg-gradient-to-t from-black/55 via-transparent to-transparent" /><span className="absolute bottom-4 left-4 bg-black/75 px-3 py-1.5 text-[0.58rem] font-black uppercase tracking-[0.12em] text-[#efcb73] backdrop-blur">{item.category}</span></div><div className="p-5"><FoodType type={item.type} protein={item.protein} /><h3 className="mt-4 font-serif text-2xl text-[#1b1711]">{item.title}</h3><p className="mt-3 text-sm leading-7 text-[#706658]">{item.copy}</p></div></article>)}</div>
        </div>
      </section>
