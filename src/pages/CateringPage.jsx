  ...
  return (
    <>
      <section className={internalHero}>
        ...
        <div className={internalHeroInner}>
          <div className="max-w-3xl">
            <p className="text-[0.7rem] font-black uppercase tracking-[0.3em] text-[#d8ab4d]">Royalties Catering</p>
            <h1 className="mt-5 font-serif text-[clamp(4rem,8vw,7.5rem)] leading-[0.86] tracking-[-0.05em]">Your occasion.<br /><span className="text-[#d8ab4d]">Our grand table.</span></h1>
            <p className="mt-7 max-w-2xl text-base leading-8 text-white/68 sm:text-lg">Bring the Royalties buffet experience to weddings, corporate events and private celebrations across Delhi NCR.</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#0a0a0a] py-20 text-white lg:py-24">
        <div className={shell}>
          <div className="grid gap-px overflow-hidden border border-white/10 bg-white/10 md:grid-cols-2 xl:grid-cols-4">
            {eventTypes.map(([Icon, title, copy]) => (
              <article key={title} className="group bg-[#0d0d0d] p-7 transition duration-500 hover:bg-[#15120c]">
                <div className="grid size-12 place-items-center border border-[#d8ab4d]/35 text-[#d8ab4d] transition group-hover:bg-[#d8ab4d] group-hover:text-black"><Icon className="size-5" /></div>
      ...
      <section className="bg-[#f3ecdf] py-24 lg:py-28">
        <div className={shell}>
          <div className="grid items-center gap-12 lg:grid-cols-[1.05fr_.95fr] lg:gap-18">
            <div>
              <p className="text-[0.68rem] font-black uppercase tracking-[0.28em] text-[#9d7328]">Built around your event</p>
              <h2 className="mt-4 font-serif text-[clamp(3rem,5vw,5.2rem)] leading-[0.94] tracking-[-0.04em] text-[#17130e]">From one buffet line to a complete food experience.</h2>
              <p className="mt-6 max-w-2xl text-base leading-8 text-[#665c4e]">The catering format can scale from a focused buffet to multiple live counters and chef-led stations. Menus can mix Indian favourites with global cuisines while keeping service coordinated around the event schedule.</p>
      ...
      <section className="bg-[#101010] py-24 text-white">
        <div className={shell}>
          <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div className="max-w-3xl"><p className="text-[0.68rem] font-black uppercase tracking-[0.28em] text-[#d8ab4d]">Multi-cuisine catering</p><h2 className="mt-4 font-serif text-[clamp(3rem,5vw,5rem)] leading-none tracking-[-0.04em]">A menu that can travel across cuisines.</h2></div><p className="max-w-md text-sm leading-7 text-white/45">Cuisine combinations can be tailored to the event instead of forcing every celebration into one fixed package.</p></div>
          <div className="mt-10 flex flex-wrap gap-2">{cuisines.map((cuisine) => <span key={cuisine} className="border border-[#d8ab4d]/25 bg-[#d8ab4d]/[.04] px-4 py-3 text-[0.62rem] font-black uppercase tracking-[0.12em] text-[#edc66c]">{cuisine}</span>)}</div>
        </div>
      </section>

      <section className="bg-[#f3ecdf] py-24 lg:py-28">
        <div className={`${shell} grid gap-8 xl:grid-cols-[.9fr_1.1fr]`}>
          <div className="bg-[#d8ab4d] p-7 text-[#151108] sm:p-9 lg:p-11">
            <Sparkles className="size-8" />
            <p className="mt-8 text-[0.65rem] font-black uppercase tracking-[0.2em] opacity-55">Plan with Royalties</p>
            <h2 className="mt-3 font-serif text-[clamp(3rem,5vw,4.7rem)] leading-[0.92] tracking-[-0.04em]">Tell us what you’re hosting.</h2>
            <p className="mt-6 max-w-lg text-sm leading-7 opacity-70">Share the event type, approximate guest count and location. The final backend will turn this into a managed catering lead with status, follow-ups and proposal tracking.</p>
            <div className="mt-9 space-y-4 border-t border-black/15 pt-7 text-sm">
              <div className="flex items-center gap-3"><MapPin className="size-4" /><span>Delhi NCR focused service area</span></div>
              <div className="flex items-center gap-3"><Users className="size-4" /><span>Small gatherings to large-format events</span></div>
              <div className="flex items-center gap-3"><ChefHat className="size-4" /><span>Buffets, live counters and custom combinations</span></div>
            </div>
          </div>
          ...
          <form onSubmit={submit} noValidate className="bg-[#0d0d0d] p-6 text-white sm:p-8 lg:p-10">
            ...
            <div className="mt-7 grid gap-5 md:grid-cols-2">
              ...
              <CustomSelect label="Guest Count" value={form.guests} options={guestOptions} icon={Users} onChange={(value) => update("guests", value)} />
              <CustomSelect label="Area" value={form.area} options={areaOptions} icon={MapPin} onChange={(value) => update("area", value)} />
