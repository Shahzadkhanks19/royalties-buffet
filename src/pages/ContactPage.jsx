          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {contactCards.map(([Icon, title, value, copy]) => (
              <article key={title} className="group border border-black/10 bg-[#fffaf2] p-6 shadow-[0_16px_36px_rgba(36,24,10,.08)] transition duration-500 hover:-translate-y-1 hover:border-[#c9983d]/55 hover:shadow-[0_24px_50px_rgba(36,24,10,.14)]">
                ...
                <p className="mt-2 break-words text-sm font-bold text-[#7b5b22]">{value}</p>
        ...
        <div className={`${shell} grid gap-8 xl:grid-cols-[1.1fr_.9fr]`}>
          <form onSubmit={handleSubmit} noValidate className="border border-white/10 bg-[#0d0d0d] p-5 sm:p-7 lg:p-9">
            <div className="flex items-start justify-between gap-6 border-b border-white/10 pb-6">
              ...
              <MessageSquareText className="size-8 shrink-0 text-[#d8ab4d]" strokeWidth={1.5} />
            </div>

            <div className="mt-7 grid gap-5 md:grid-cols-2">
              <label className="block"><span className="mb-2 block text-[0.58rem] font-black uppercase tracking-[0.14em] text-white/45">Name</span><input value={form.name} onChange={(event) => update("name", event.target.value)} aria-invalid={Boolean(errors.name)} aria-describedby={errors.name ? "contact-name-error" : undefined} placeholder="Your name" className={fieldClass(errors.name)} /><span id="contact-name-error"><ErrorText message={errors.name} /></span></label>
            ...
            <button type="submit" className={`${buttonGold} mt-7 w-full gap-3 sm:w-auto`}>Send Message <Send className="size-4" /></button>
            {submitted ? <div className="mt-6 flex items-start gap-3 border border-[#d8ab4d]/30 bg-[#d8ab4d]/8 p-4" role="status"><Check className="mt-0.5 size-4 shrink-0 text-[#d8ab4d]" /><p className="text-sm leading-6 text-white/72">Message validated successfully. It is ready to connect to the contact backend.</p></div> : null}
          </form>
          ...
          <aside className="space-y-5">
            ...
            <div className="relative min-h-[360px] overflow-hidden border border-white/10 bg-black">
              <SafeImage src="https://images.unsplash.com/photo-1587474260584-136574528ed5?auto=format&fit=crop&w=1400&q=88" alt="India Gate in New Delhi" loading="lazy" className="absolute inset-0 h-full w-full object-cover opacity-55" />
