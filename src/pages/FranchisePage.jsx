    ...
    <>
      <section className={internalHero}>
        ...
        <div className={internalHeroInner}>
          <div className="max-w-3xl">
            <p className="text-[0.7rem] font-black uppercase tracking-[0.3em] text-[#d8ab4d]">Franchise With Royalties</p>
            <h1 className="mt-5 font-serif text-[clamp(4rem,8vw,7.5rem)] leading-[0.86] tracking-[-0.05em]">Build the next<br /><span className="text-[#d8ab4d]">royal table.</span></h1>
            <p className="mt-7 max-w-2xl text-base leading-8 text-white/68 sm:text-lg">Partner with a premium buffet concept designed around scale, celebration, live food experiences and a recognizable hospitality identity.</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#f3ecdf] py-24 lg:py-28">
        <div className={`${shell} grid gap-12 lg:grid-cols-[.9fr_1.1fr] lg:items-center`}>
          <div><p className="text-[0.68rem] font-black uppercase tracking-[0.28em] text-[#9d7328]">The opportunity</p><h2 className="mt-4 font-serif text-[clamp(3rem,5vw,5.2rem)] leading-[0.94] tracking-[-0.04em] text-[#17130e]">A format built for high-energy dining.</h2><p className="mt-6 text-base leading-8 text-[#695f51]">Royalties combines a buffet-led dining model with live counters, celebrations, group dining and catering extensions. The franchise proposition is designed around a strong guest experience rather than a simple restaurant licence.</p><p className="mt-4 text-base leading-8 text-[#695f51]">The current growth focus is Delhi NCR, with the format positioned for dense residential, corporate and destination dining catchments.</p></div>
          <div className="grid gap-px bg-[#c9a45d]/30 sm:grid-cols-2">{support.map(([Icon, title, copy]) => <article key={title} className="group bg-[#fffaf2] p-7 transition duration-500 hover:-translate-y-1 hover:bg-white"><div className="grid size-11 place-items-center bg-[#17130e] text-[#d8ab4d]"><Icon className="size-5" /></div><h3 className="mt-6 font-serif text-2xl text-[#1b1711]">{title}</h3><p className="mt-3 text-sm leading-7 text-[#6e6457]">{copy}</p></article>)}</div>
        </div>
      </section>

      <section className="bg-[#090909] py-24 text-white lg:py-28">
        <div className={shell}>
          <div className="max-w-3xl"><p className="text-[0.68rem] font-black uppercase tracking-[0.28em] text-[#d8ab4d]">Ideal partner profile</p><h2 className="mt-4 font-serif text-[clamp(3rem,5vw,5rem)] leading-none tracking-[-0.04em]">Right market. Right operator. Right experience.</h2></div>
          <div className="mt-12 grid gap-4 md:grid-cols-2 xl:grid-cols-4">{["Strong local market understanding", "Commitment to hospitality quality", "Ability to build and lead a team", "Long-term brand-building mindset"].map((item) => <div key={item} className="border border-white/10 bg-white/[0.02] p-6"><BadgeCheck className="size-5 text-[#d8ab4d]" /><p className="mt-5 font-serif text-xl">{item}</p></div>)}</div>
        </div>
      </section>

      <section className="bg-[#f3ecdf] py-24 lg:py-28">
        <div className={shell}>
          <div className="text-center"><p className="text-[0.68rem] font-black uppercase tracking-[0.28em] text-[#9d7328]">Partnership journey</p><h2 className="mt-4 font-serif text-[clamp(3rem,5vw,5rem)] leading-none tracking-[-0.04em] text-[#17130e]">From first conversation to opening night.</h2></div>
          <div className="mt-12 grid gap-3 md:grid-cols-3 xl:grid-cols-6">{journey.map((step, index) => <div key={step} className="relative border border-black/10 bg-[#fffaf2] p-5"><span className="font-serif text-4xl text-[#c89b43]/35">0{index + 1}</span><p className="mt-5 text-sm font-black uppercase tracking-[0.08em] text-[#2c2419]">{step}</p>{index < journey.length - 1 ? <div className="absolute -right-2 top-1/2 z-10 hidden size-4 rotate-45 border-r border-t border-[#c89b43]/40 bg-[#f3ecdf] xl:block" /> : null}</div>)}</div>
        </div>
      </section>
