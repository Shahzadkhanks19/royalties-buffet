...
import { buttonGold, buttonOutline, navItems, shell } from "../../config/site";
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 bg-[#070707]/95 text-white shadow-[0_12px_45px_rgba(0,0,0,.28)] backdrop-blur-xl">
      <div className="hidden border-b border-[#d8ab4d]/15 bg-[#050505] text-[0.62rem] font-semibold tracking-[0.07em] text-white/55 lg:block">
        <div className={`${shell} flex min-h-8 items-center justify-between`}><div className="flex items-center gap-7"><span>Jodhpur, Rajasthan</span><span className="text-white/20">|</span><span>12:00 PM - 11:00 PM</span></div><div className="flex items-center gap-3"><span>Follow Us:</span><SocialLinks /></div></div>
      </div>
      <div className={`${shell} flex min-h-[76px] items-center justify-between gap-5`}>
        ...
        <nav className="hidden items-center gap-6 xl:flex" aria-label="Primary navigation">{navItems.map(([label, to]) => <NavLink key={to} to={to} className={({ isActive }) => `group relative py-6 text-[0.66rem] font-bold uppercase tracking-[0.11em] transition ${isActive ? "text-[#e9c66d]" : "text-white/78 hover:text-white"}`}>{({ isActive }) => <>{label}<span className={`absolute bottom-3 left-1/2 h-px -translate-x-1/2 bg-[#d8ab4d] transition-all duration-300 ${isActive ? "w-full" : "w-0 group-hover:w-full"}`} /></>}</NavLink>)}</nav>
        <div className="hidden items-center gap-3 sm:flex"><NavLink to="/reservation" className={buttonOutline}>Reserve</NavLink><NavLink to="/reservation" className={buttonGold}>Book a Table</NavLink></div>
