...
export default function AdminLayout() {
  ...
  const sidebar = (
    <aside className="flex h-full w-72 flex-col border-r border-white/10 bg-[#0a0a0a] text-white">
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto p-4">{nav.map(([Icon, label, to]) => <NavLink end={to === "/admin"} key={to} to={to} onClick={() => setOpen(false)} className={({ isActive }) => `flex min-h-12 items-center gap-3 border px-4 text-sm font-bold transition ${isActive ? "border-[#d8ab4d]/40 bg-[#d8ab4d]/10 text-[#efcb73]" : "border-transparent text-white/58 hover:border-white/10 hover:bg-white/[0.03] hover:text-white"}`}><Icon className="size-4" />{label}</NavLink>)}</nav>
