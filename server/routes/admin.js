...
for(const[resource,config]of Object.entries(workflow)){
  ...
  router.patch(`/${resource}/:id`,requireAdmin,async(req,res)=>{ensureDatabase();const item=await config.model.findById(validId(req.params.id));if(!item)throw new ApiError(404,"Record not found.");if(req.body?.status!==undefined){if(!config.statuses.includes(req.body.status))throw new ApiError(400,"Invalid status.");if(resource==="reservations"&&item.status!==req.body.status)item.history.push({action:"status-changed",fromStatus:item.status,toStatus:req.body.status,note:cleanText(req.body?.historyNote,500)});item.status=req.body.status;}if(req.body?.adminNotes!==undefined)item.adminNotes=cleanText(req.body.adminNotes,3000);if(req.body?.isArchived!==undefined)item.isArchived=Boolean(req.body.isArchived);await item.save();res.json({ok:true,item});});
